...
namespace ImagePoster4DTF {
	public class VisibilityAnimation {
		...
		private static void OnAnimationTypePropertyChanged(DependencyObject dependencyObject,
			...
			if (!(dependencyObject is FrameworkElement frameworkElement)) return;

			// If AnimationType is set to True on this framework element,
			if (GetAnimationType(frameworkElement) != AnimationType.None) // Add this framework element to hooked list
				HookVisibilityChanges(frameworkElement);
			else // Otherwise, remove it from the hooked list
		...
		private static object CoerceVisibility(DependencyObject dependencyObject, object baseValue) {
			...
			if (!(dependencyObject is FrameworkElement frameworkElement)) return baseValue;
				...
				if (visibility == Visibility.Visible) {
					...
					if (BindingOperations.IsDataBound(frameworkElement, UIElement.VisibilityProperty)) {
						...
						BindingOperations.SetBinding(frameworkElement, UIElement.VisibilityProperty,
							bindingValue ?? throw new NullReferenceException());
					}
					else {
